...
class Sfmt
{
    ...
    protected int[]x=new int[40]; /* ��ԃe�[�u�� */

    protected int[]GetParity()
    { return new int[]{ 0x00000001,0x00000000,0x00000000,0x20000000 }; }

    public String IdString() {
        return"SFMT-1279:7-14-3-5-1:f7fefffd-7fefcfff-aff3ef3f-b5ffff7f";
    }

    public void gen_rand_all() {
        int a=0,b=28,c=32,d=36,y; int[]p=x;

        do {
            y=p[a+3]^(p[a+3]<<24)^(p[a+2]>>>8)^((p[b+3]>>>5)&0xb5ffff7f);
            p[a+3]=y^(p[c+3]>>>8)^(p[d+3]<<14);
            y=p[a+2]^(p[a+2]<<24)^(p[a+1]>>>8)^((p[b+2]>>>5)&0xaff3ef3f);
            p[a+2]=y^((p[c+2]>>>8)|(p[c+3]<<24))^(p[d+2]<<14);
            y=p[a+1]^(p[a+1]<<24)^(p[a]>>>8)^((p[b+1]>>>5)&0x7fefcfff);
            p[a+1]=y^((p[c+1]>>>8)|(p[c+2]<<24))^(p[d+1]<<14);
            y=p[a]^(p[a]<<24)^((p[b]>>>5)&0xf7fefffd);
    ...
    void period_certification() {
        ...
        index=40; range=0; normal_sw=0; coin_bits=0; byte_pos=0;
    ...
    /*synchronized*/ public int NextMt() {
        if (index==40) { gen_rand_all(); index=0; }
        return x[index++];
    }
